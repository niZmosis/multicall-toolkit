...
export class Erc1155ContractFactory
  extends MulticallProviderBase
  implements Erc1155Contract.Contract
{
  protected _contractDetail: ContractDetail

  protected _contract: Erc1155Contract.ContractContext

  protected _methodNames: Erc1155Contract.MethodNameMap
  ...
  constructor(
    ...
    this._contract =
      this._multicallProvider.getContract<Erc1155Contract.ContractContext>(
        this._contractDetail,
      )
  ...
  public get erc1155Contract(): Erc1155Contract.ContractContext {
    return this._contract
  }

  /** Get the method names */
  public get methodNames(): Erc1155Contract.MethodNameMap {
  ...
  protected async callContractMethod<T>(
    methodName: Erc1155Contract.MethodNames,
    values?: any[],
  ): Promise<T> {
    const contractMethodName = this._methodNames[
      methodName
    ] as keyof Erc1155Contract.ContractContext
  ...
  protected encodeFunctionData(
    ...
    return this._contract.interface.encodeFunctionData(
      this._methodNames[methodName] as Erc1155Contract.MethodNames,
      values,
    )
  }
   ...
   */
  protected prepareCallContext<TMethod extends keyof Erc1155Contract.Contract>(
    methodName: TMethod,
    methodParameters: any[] = [],
  ): MethodCall<Erc1155Contract.Contract, TMethod> {
    const contractMethodName = this._methodNames[
      methodName
    ] as keyof Erc1155Contract.Contract

    if (typeof this._contract[contractMethodName] === 'function') {
      return {
        methodName,
        methodParameters: methodParameters ?? [],
      } as MethodCall<Erc1155Contract.Contract, TMethod>
    } else {
      throw new MulticallError(
        `Method ${methodName} does not exist on the contract`,
        ErrorCodes.functionArgumentError,
  ...
  async call<
    TCalls extends Record<
      string,
      DiscriminatedMethodCalls<Erc1155Contract.Contract>[MethodNames<Erc1155Contract.Contract>]
    >,
  >(
    calls: TCalls,
    ...
    originContext: ContractResults<
      ...
      TCalls
    >['originContext']
    results: ContractResults<Erc1155Contract.Contract, TCalls>['results']
  }> {
    return super.multicall<Erc1155Contract.Contract, TCalls>(calls, options)
  }
  ...
  public balanceOfCallContext(
    account: string,
    id: BigNumberish,
  ): MethodCall<Erc1155Contract.Contract, 'balanceOf'> {
  ...
  public balanceOfBatchCallContext(
    accounts: string[],
    ids: BigNumberish[],
  ): MethodCall<Erc1155Contract.Contract, 'balanceOfBatch'> {
  ...
  public isApprovedForAllCallContext(
    account: string,
    operator: string,
  ): MethodCall<Erc1155Contract.Contract, 'isApprovedForAll'> {
  ...
  ): MethodCall<Erc1155Contract.Contract, 'supportsInterface'> {
  ...
  public uriCallContext(
    id: BigNumberish,
  ): MethodCall<Erc1155Contract.Contract, 'uri'> {
